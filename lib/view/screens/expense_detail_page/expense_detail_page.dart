...
class ExpanseDetailPage extends StatefulWidget {
  const ExpanseDetailPage({super.key});

  @override
  State<ExpanseDetailPage> createState() => _ExpanseDetailPageState();
}

class _ExpanseDetailPageState extends State<ExpanseDetailPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      bloc.add(GetExpenseDetailsEvent(page: 1));
    });
  }
  ...
  void _fetchMoreData() {
    ...
    context
        ...
        .add(GetExpenseDetailsEvent(page: currentPage));
  }
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          ...
          context
              .read<IncomeExpenseBloc>()
              .add(GetExpenseDetailsEvent(page: 1));
        },
        child: Column(
          children: [
              ...
              child: BlocConsumer<IncomeExpenseBloc, IncomeExpenseState>(
                listener: (context, state) {
                  ...
                  if (state is ExpenseFetchSuccess) {
                    log('expense details fetched: ${state.expenseDetails.length} items');
                    setState(() {
                      _isFetchingMore = false;
                    });
                ...
                builder: (context, state) {
                  ...
                  if (state is ExpenseFetchSuccess) {
                    final expenseDetails = state.expenseDetails;
                    log('Rendering ${expenseDetails.length} expense details');

                    if (expenseDetails.isEmpty) {
                      return const SingleChildScrollView(
                        physics: AlwaysScrollableScrollPhysics(),
                        child: Center(child: Text("No Expense Details Found")),
                      );
                    }

                    return ScrollableTable(
                      columnLabels: const ['Date', 'Expense Type', 'Amount'],
                      rowData: expenseDetails
                          .map((item) => [
                                item.date,
                                item.expenseType,
                  ...
                  if (state is ExpenseLoadingMore) {
                    final expenseDetails = state.expenseDetails;
                    log('Rendering ${expenseDetails.length} expense details while loading more');
